...
export class AudioLoaderCompatState {
  ...
  private setAudioLoaderEnabled(enabled: boolean) {
    let audioLoader = (window as any).AUDIOLOADER_MENUMUSIC;
    if (audioLoader) {
...
const AudioLoaderCompatStateContext = createContext<PublicAudioLoaderCompatStateContext>(null as any);
export const useAudioLoaderCompatState = () => useContext(AudioLoaderCompatStateContext);
...
export const AudioLoaderCompatStateContextProvider: FC<ProviderProps> = ({
  ...
  const setGamesRunning = (gamesRunning: Number[]) => AudioLoaderCompatStateClass.setGamesRunning(gamesRunning);
  const setOnThemePage = (onAppPage: boolean) => AudioLoaderCompatStateClass.setOnThemePage(onAppPage);
